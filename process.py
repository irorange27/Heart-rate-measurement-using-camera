...
class Process(object):
    ...
    def run(self):
        ...
        if L == self.buffer_size:
            ...
            # 使用 Signal_processing.fft 封装（优先 C 实现，回退 numpy）
            fft_vals, freqs_vals = self.sp.fft(norm, self.fps)
            self.fft = fft_vals
            self.freqs = freqs_vals

            if self.fft is None or len(self.fft) == 0:
                return True
            idx2 = np.argmax(self.fft)#max in the range can be HR
